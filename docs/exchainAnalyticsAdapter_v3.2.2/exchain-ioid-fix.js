...
window.exchainPrebidModule = {
  ...
  init: function(config = {}) {
    ...
    try {
      ...
      window.pbjs.onEvent('auctionInit', this.onAuctionInit.bind(this));
      
      // Keep beforeRequestBids as fallback for safety
      window.pbjs.onEvent('beforeRequestBids', this.onBeforeRequestBids.bind(this));
  ...
  injectGlobalIOID: function(ioid) {
    if (!window.pbjs || !ioid) return;

    try {
      // Get current ORTB2 configuration
      const ortb2 = window.pbjs.getConfig('ortb2') || {};
      ...
      window.pbjs.setConfig({ ortb2 });
      console.log(`ExChain Analytics v${MODULE_VERSION}: Successfully injected IOID:`, ioid);
...
window.pbjs.installedModules = window.pbjs.installedModules || [];

// Add the module to installedModules
if (!window.pbjs.installedModules.includes(MODULE_NAME)) {
    window.pbjs.installedModules.push(MODULE_NAME);
    console.log(`ExChain Analytics v${MODULE_VERSION}: Module registered with Prebid.js`);
}

// Auto-initialize when Prebid loads
window.pbjs.que.push(function() {
    if (window.exchainPrebidModule && typeof window.exchainPrebidModule.init === 'function') {
        window.exchainPrebidModule.init();
        console.log(`ExChain Analytics v${MODULE_VERSION}: Module initialized via queue`);
    } else {
        console.error(`ExChain Analytics v${MODULE_VERSION}: Module not found or init method missing`);
    }
});
